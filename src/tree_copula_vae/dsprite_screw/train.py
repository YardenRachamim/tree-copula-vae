...
from tree_copula_vae.dsprite_screw.config import Config, ModelType
from tree_copula_vae.dsprite_screw.data import ScrewDataModule
from tree_copula_vae.dsprite_screw.vae import CopulaVAE, MeanFieldVAE
from tree_copula_vae.fashion_mnist.callbacks import AnnealAttributeCallback, KLAnealingCallback


def build_model(config: Config):
    model_config = config.model
    if model_config.model_type == ModelType.COPULA_VAE:
        return CopulaVAE(
            ...
            use_nf_prior=model_config.use_nf_prior,
            ...
            use_copula_decoder=model_config.use_copula_decoder,
            decoder_rank=model_config.decoder_rank,
        )
    if model_config.model_type == ModelType.MEAN_FIELD_VAE:
        return MeanFieldVAE(
            latent_dim=model_config.latent_dim,
            lr=model_config.learning_rate,
            kl_coeff=model_config.kl_coeff,
...
def build_datamodule(config: Config) -> ScrewDataModule:
    return ScrewDataModule(
...
def _resume_state(config: Config) -> Tuple[Optional[str], Optional[bool]]:
    ...
    if checkpoint.resume_training:
        if checkpoint.run_id is None:
            raise ValueError("checkpoint.run_id is required when resuming training")
...
def run(config: Config, config_path: Path) -> None:
    ...
    experiment_name = "{} Screw HD{} {} {}".format(
